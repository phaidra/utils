...
sub get_keyword_nodes {
  ...
  foreach my $sf (@{$field->{subfield}}){
    ...
    if($lab eq 'f' || $lab eq 'n'){ # rasta 20181218 I'm adding 'n', it contains 'Themakarte'
      if(defined($gnd_id)){
...
sub get_fixed_license_node {
  ...
  return {
    ...
    "input_type" => "input_text",
    "ui_value" => "http://creativecommons.org/licenses/by/4.0/",
    "attributes" => [
      {
        "xmlname" => "type",
        "input_type" => "select",
